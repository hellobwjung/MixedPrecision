...
def load_checkpoint_if_exists(model, model_dir, model_name, ckpt_name=None):
    ...
    if (ckpt_name != None ) and \
       (os.path.isfile(trained_model_file_name) and os.path.exists(trained_model_file_name)):
        ...
        idx = ckpt_name.rfind(model_name)
        prev_epoch = int(ckpt_name[idx-6:idx-1])
        prev_loss = float(ckpt_name.split('_')[-1][:-3])
        ...
        trained_weights = glob.glob(os.path.join(model_dir, model_name, '*.h5' ))
...
class bwutils():
    ...
    def __init__(self,
                 ...
                 input_bits =16,
                 input_bias = True,
                 card_type='same_noise',  # 'nocard', n2n', 'same_noise'
                 use_unprocess=False,
                 loss_scale = 1,
    ...
    def get_patternized_3ch_image(self, image):
        ...
        print(image[...,:3].shape, tf_RGB.shape)
    ...
    def make_data(self, image):



        # scale image --> normalized --> [0, 1]
        image = self.scale_by_input_max(image)
        # before_patternized = self.scale_by_input_max(before_patternized)

        # add noise - noise2noise
        if self.card_type in ['noise2noise']:
            before_patternized = tf.identity(image)
            image = self.add_noise_batch(image)
            before_patternized = self.add_noise_batch(before_patternized)
        elif self.card_type in ['same_noise']:
            image = self.add_noise_batch(image)
            before_patternized = tf.identity(image)

        # clip value
        image               = tf.clip_by_value(image, 0, 1)
        before_patternized  = tf.clip_by_value(before_patternized, 0, 1)

        # patternized
        patternized = self.get_patternized(before_patternized, self.input_type)

        if self.input_bias:
            image = (image * 2) - 1
            patternized = (patternized * 2) - 1

        return patternized, image

    def parse_tfrecord_train(self, example):

        image = self.get_image_from_single_example(example, key='image', num_channels=3)

        image = self.data_augmentation(image)

        patternized, image = self.make_data(image)
    ...
    def dataset_input_fn(self, params):
        ...
        if params['mode'] == tf.estimator.ModeKeys.TRAIN :
            dataset = dataset.map(self.parse_tfrecord_train,
...
class TensorBoardImage(Callback):
    ...
    def write_image(self, tag, epoch):
        ...
        for idx,  (x, y) in enumerate(self.dataloader):
            ...
            # x = tf.math.pow(x, 1/2.2)
            # y = tf.math.pow(y, 1/2.2)
            # pred = tf.math.pow(pred, 1/2.2)
            # diff   = tf.math.abs(y-pred)
